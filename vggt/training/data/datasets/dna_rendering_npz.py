...
from vggt.utils.load_fn import convert_extrinsics_to_relative_tensor, adjust_intrinsic, adjust_intrinsic_batch


def dna_collate_fn(batch):
    """
    自定义 collate 函数处理 DNA 数据集
    batch: 包含多个样本的列表，每个样本是 read_dna_npz_entry 的返回值
    """
    # 初始化结果字典
    collated_batch = {
        'image_bytes': [],
    ...
    for sample in batch:
        ...
        # 对于 image_bytes 和 bg_color，直接添加到列表（不堆叠）
