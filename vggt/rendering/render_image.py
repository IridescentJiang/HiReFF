...
from vggt.rendering.gs_ras import gaussian_render, get_render_cam
from torch.profiler import profile, record_function, ProfilerActivity
import numpy as np
from scipy.spatial.transform import Slerp
from scipy.spatial.transform import Rotation as R
from vggt.rendering.camera_mapping import mapping_camera
import json
import os
from torchvision.utils import save_image
import numpy
import imageio
from vggt.utils.voxelizaton_fusion import voxelizaton_with_fusion
...
def render_images_infer(model, loader, config, rank=None, inter_view=0, type="Interpolate"):
    ...
    with torch.no_grad():
        ...
        images = batch["images"].to(device, non_blocking=True, dtype=torch.float32)

        # 混合精度前向（保持与训练一致）
        with torch.cuda.amp.autocast(enabled=config.amp_enabled, dtype=torch.bfloat16):
            preds = model(images)

        # 将渲染视角线性插值出新视角
        if type == "Interpolate":
            preds["pose_enc"] = interpolate_pose(preds["pose_enc"], inter_view=inter_view)
        elif type == "All":
            preds["pose_enc"] = get_all_poses(preds["images"], if_mapping=False, if_render_video=False)
        else:
            raise ValueError('render type must be "Interpolate" or "All".')

        rendered_images, _ = batch_render_images(preds, wo_bg=True)

    return rendered_images.detach().cpu().clone()


def get_all_poses(images, if_mapping=False, if_render_video=False):
    """
    利用camera_mapping的predict_new_camera，将dna_rendering_cam_para.json中所有的相机参数都转换到VGGT的预测空间
    """
    ...
    if if_render_video:
        cam_para = "dna_rendering_cam_para_render_video.json"
    else:
    ...
    with open(dna_rendering_cam_para_path, "r") as f:
        true_cameras = json.load(f)

    all_poses = []
    for i in range(len(true_cameras["intrinsic"])):
        true_intrinsic = np.array(true_cameras["intrinsic"][i])
        true_extrinsic = np.array(true_cameras["extrinsic"][i])

        if if_mapping:
            pred_intrinsic, pred_extrinsic = mapping_camera(true_intrinsic, true_extrinsic)
        else:
            pred_intrinsic = true_intrinsic.copy()
            # 将主点设置为图像中心
            pred_intrinsic[0, 2] = W / 2.0  # cx
            pred_intrinsic[1, 2] = H / 2.0  # cy

            # 复制原始外参矩阵
            pred_extrinsic = true_extrinsic.copy()
            # 获取平移向量部分 (3x1向量)
            T = pred_extrinsic[:3, 3]
            # 缩放平移向量
            T *= radio
            # 更新外参矩阵中的平移分量
            pred_extrinsic[:3, 3] = T

        # 将预测的内外参转换为torch张量
        pred_intrinsic = torch.tensor(pred_intrinsic, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        pred_extrinsic = torch.tensor(pred_extrinsic, dtype=torch.float32).unsqueeze(0).unsqueeze(0)

        # 将预测的内外参转换为VGGT的pose encoding格式
        encoded_pose = extri_intri_to_pose_encoding(pred_extrinsic, pred_intrinsic, (H, W)).squeeze(0)

        all_poses.append(encoded_pose)

    return torch.stack(all_poses, dim=1)
...
def adding_mapped_poses(preds, intrinsics, extrinsics):
    """
    利用camera_mapping的mapping_camera，将输入的相机参数都转换到VGGT的预测空间
    """

    B, V, C, H, W = preds["images"].shape

    new_poses = []
    for b_id in range(B):
        for v_id in range(V):
            true_intrinsic = np.array(intrinsics[b_id][v_id])
            true_extrinsic = np.array(extrinsics[b_id][v_id])
    ...
    new_poses_torch = torch.stack(new_poses, dim=1).to(preds["pose_enc"].device)
    preds["pose_enc"] = torch.cat([preds["pose_enc"], new_poses_torch], dim=1)
...
def batch_render_images(pred, wo_bg=True, sr_image_size=None, voxel_size=1e-3, voxelizaton_fusion=False,
                        render_depth=False, bg_color=None, render_mode='mipsplat'):
    """
    批量渲染所有图像（优化显存使用）
    严格使用 get_render_cam 和 gaussian_render
    """
    ...
    if wo_bg:
        masks = rearrange(pred["masks"], "b v h w c-> b (v h w) c").contiguous() > 0.5
        for key in ["xyz", "colors", "rotations", "scales", "opacities"]:
            pred["flat_gs"][key].mul_(masks)  # 原地操作，避免创建新张量

    # 准备批量渲染数据
    if voxelizaton_fusion:
        xyz, colors, rotations, scales, opacities = voxelizaton_with_fusion(
            pred["flat_gs"]["xyz"],
            pred["flat_gs"]["colors"],
            pred["flat_gs"]["rotations"],
            pred["flat_gs"]["scales"],
            pred["flat_gs"]["opacities"],
            voxel_size
        )

        if render_mode == 'mipsplat':
            rendered_images = vectorized_gaussian_render(
        ...
        elif render_mode == 'gsplat':
            rendered_images, rendered_depth = vectorized_gaussian_render_gsplat(
                pose_enc=pred["pose_enc"],
                images=pred["images"],
                xyz=xyz,
                colors=colors,
        ...
        if render_mode == 'mipsplat':
            rendered_images = vectorized_gaussian_render(
                pose_enc=pred["pose_enc"],
                images=pred["images"],
                xyz=pred["flat_gs"]["xyz"],
                colors=pred["flat_gs"]["colors"],
                rotations=pred["flat_gs"]["rotations"],
                scales=pred["flat_gs"]["scales"],
                opacities=pred["flat_gs"]["opacities"],
                image_size=(H, W),
                batch_size=B,
                view_size=V,
                bg_color=bg_color
...
def vectorized_gaussian_render(pose_enc, images, xyz, colors, rotations, scales, opacities, image_size, batch_size,
                               view_size, bg_color):
    """
    优化显存使用的向量化高斯渲染
    """
    ...
    for b_idx in range(batch_size):
        for v_idx in range(view_size):
            R = extrinsics[b_idx, v_idx, :3, :3]
            T = extrinsics[b_idx, v_idx, :3, 3]
            intr = intrinsics[b_idx, v_idx]

            cam_param = get_render_cam(intr=intr, R=R, T=T, h=H, w=W)

            render_image = gaussian_render(
                cam_param=cam_param,
                pts_xyz=xyz[b_idx],
                pts_rgb=colors[b_idx],
                rotations=rotations[b_idx],
                scales=scales[b_idx],
                opacity=opacities[b_idx],
                bg_color=bg_color[b_idx, v_idx]
            )
            render_images.append(render_image)
    return torch.stack(render_images, dim=0)


def vectorized_gaussian_render_gsplat(pose_enc, images, xyz, colors, rotations, scales, opacities, image_size,
                                      batch_size, view_size, render_depth, bg_color):
    """
    优化显存使用的向量化高斯渲染
    """
    H, W = image_size
    ...
    K = colors.shape[-1] // 3
    if K > 1:
        colors = colors.reshape(colors.shape[0], colors.shape[1], K, 3)

    render_images_list = []
    render_depth_list = []

    sh_degree = (int(sqrt(K)) - 1) if K > 1 else None
    ...
    for b_idx in range(batch_size):
        ...
        if render_depth:
            rendering, *_ = rasterization(
                xyz[b_idx:b_idx + 1],  # 保持 [1, N, 3] 形状
                rotations[b_idx:b_idx + 1],
                scales[b_idx:b_idx + 1],
                opacities[b_idx:b_idx + 1].squeeze(-1),  # [1, 1, N] -> [1, N]
                colors[b_idx:b_idx + 1],
                current_extrinsic_extend.unsqueeze(0),  # [1, B*V, 4, 4]
                intrinsics[b_idx:b_idx + 1],
                W, H,
                sh_degree=sh_degree,
            ...
            render_image = rendering.clamp(0.0, 1.0)
            # 直接获取 RGB 图像 (避免 torch.split)
            render_images_list.append(render_image)
...
def normalize_tensor_minmax(tensor: torch.Tensor) -> torch.Tensor:
    """
    使用最小-最大缩放将张量归一化到[0,1]范围
...
def print_tensor_stats(image: torch.Tensor, name="Image"):
    """
    打印张量的统计信息
    """
    # 确保是PyTorch张量并且计算在CPU上
    if not torch.is_tensor(image):
        image = torch.tensor(image)
    image = image.detach().cpu()

    # 计算统计数据
    max_value = torch.max(image).item()
    min_value = torch.min(image).item()
    mean_value = torch.mean(image.float()).item()

    # 创建格式化的输出字符串
    stats_str = (f"{name} 统计信息:\n"
                 f"  最大值: {max_value:.6f}\n"
                 f"  最小值: {min_value:.6f}\n"
                 f"  平均值: {mean_value:.6f}")
...
def save_rendered_images(images: numpy.array, save_path: str, epoch=0, start_id=None):
    """
    批量保存渲染图像到指定路径
    Args:
