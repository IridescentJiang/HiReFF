...
class XCA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # self.attn_drop = attn_drop if FLASH_ATTN_AVAILABLE else nn.Dropout(attn_drop)
    ...
    # def forward_flash(self, x):
    #     B, N, C = x.shape
    #     qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads)
        
    #     attn = flash_attn_qkvpacked_func(qkv, dropout_p=self.attn_drop)  # (batch_size, seqlen, nheads, headdim)
