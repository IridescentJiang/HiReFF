...
model = VGGT.from_checkpoint("./checkpoints/best_model_epoch_34_loss_0.3269.pt")
...
def gradio_demo(
    ...
    plyfile = os.path.join(
        target_dir,
        f"glbscene_{conf_thres}_{frame_filter.replace('.', '_').replace(':', '').replace(' ', '_')}_maskb{mask_black_bg}_maskw{mask_white_bg}_cam{show_cam}_sky{mask_sky}_pred{prediction_mode.replace(' ', '_')}.ply",
    )
