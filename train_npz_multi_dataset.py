...
class TrainingConfig:
    ...
    mix_balance = True  # 是否对 mix 模式下各子数据集做均衡采样
    mix_balance_mode = "downsample"  # downsample or upsample or weighted
    mix_balance_seed = 42
    mix_balance_val = False  # 是否也对验证集做均衡，默认保持原始分布
    mix_dataset_weights = {"dna": 1.0, "zju": 1.0, "mvhuman": 1.0}  # weighted 模式下的数据集权重
    mix_weighted_target_total = None  # weighted 模式下总采样目标，None 表示使用当前总样本数

    data_root_prefix = "/home/china/lab/VGGT_human/dna_rendering/data_processing/overfitting"
    ...
    # 兼容旧配置：当只训练单一数据集时可直接使用 data_root
    if dataset_mode == "single":
        if single_dataset == "dna":
            ...
            raise ValueError(f"Unsupported single_dataset: {single_dataset}. Expected 'dna' or 'zju' or 'mvhuman'.")
    ...
    checkpoint = "checkpoints/stage_3.pt"
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        for image_bytes in batch["image_bytes"]:
            ...
            for image_byte in image_bytes:
                img = torchvision.io.decode_jpeg(image_byte, mode=torchvision.io.ImageReadMode.RGB,
                                                 device=torch.device(device)).float() / 255.0  # (3, H, W)
                decoded_images.append(img)
            sr_image = torch.stack(decoded_images, dim=0)  # (n_views, 3, H, W)
            sr_image_list.append(sr_image)
        sr_images = torch.stack(sr_image_list, dim=0)  # (batch_size, n_views, 3, H, W)

        sr_masks = batch["masks"].to(device)
        bg_color = batch["bg_color"].view(1, 1, 3, 1, 1).expand_as(sr_images).to(device)  # 调整形状并扩展
        sr_images_wo_bg = sr_images * sr_masks + (1 - sr_masks) * bg_color  # (n_views, 3, H, W)

        masks = interpolate_images(
            sr_masks, target_size=(config.img_size, config.img_size)
        )
        images = interpolate_images(
            sr_images, target_size=(config.img_size, config.img_size)
        )
        images_wo_bg = interpolate_images(
            sr_images_wo_bg, target_size=(config.img_size, config.img_size)
        )
        if sr_images_wo_bg.shape[-2] != config.sr_img_size or sr_images_wo_bg.shape[-1] != config.sr_img_size:
            sr_images_wo_bg = interpolate_images(
                sr_images_wo_bg, target_size=(config.sr_img_size, config.sr_img_size)
            )
        ...
        if config.multiview_supervise > 0:
            ...
            for supervise_image_bytes in batch["supervise_image_bytes"]:
                decoded_supervise_images = []
                for supervise_image_byte in supervise_image_bytes:
                    supervise_img = torchvision.io.decode_jpeg(supervise_image_byte, mode=torchvision.io.ImageReadMode.RGB,
                                                     device=torch.device(device)).float() / 255.0  # (3, H, W)
                    decoded_supervise_images.append(supervise_img)
                sr_supervise_image = torch.stack(decoded_supervise_images, dim=0)  # (n_views, 3, H, W)
                sr_supervise_image_list.append(sr_supervise_image)
            sr_supervise_images = torch.stack(sr_supervise_image_list, dim=0)  # (batch_size, n_views, 3, H, W)

            sr_supervise_masks = batch["supervise_masks"].to(device)
            sr_supervise_images_wo_bg = sr_supervise_images * sr_supervise_masks + (1 - sr_supervise_masks) * bg_color  # (n_views, 3, H, W)
            supervise_images_wo_bg = interpolate_images(
                sr_supervise_images_wo_bg, target_size=(config.img_size, config.img_size)
            )  # (n_views, 3, 518, 518)
            supervise_masks = interpolate_images(
                sr_supervise_masks, target_size=(config.img_size, config.img_size)
            )  # (n_views, 3, 518, 518)
            if sr_supervise_images_wo_bg.shape[-2] != config.sr_img_size or sr_supervise_images_wo_bg.shape[-1] != config.sr_img_size:
                sr_supervise_images_wo_bg = interpolate_images(
                    sr_supervise_images_wo_bg, target_size=(config.sr_img_size, config.sr_img_size)
                )

            targets.update({
                "supervise_images": supervise_images_wo_bg,
        ...
        if rank == 0 and batch_idx % 20 == 0:
            pred_vis = rendered_images[:, :3, ...] if rendered_images.size(1) > 3 else rendered_images
            gt_vis = targets["sr_images"].reshape(-1, *targets["sr_images"].shape[2:])
            combined_output = torch.cat([pred_vis, gt_vis], dim=-1)
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if config.multiview_supervise > 0:
                ...
                for supervise_image_bytes in batch["supervise_image_bytes"]:
                    decoded_supervise_images = []
                    for supervise_image_byte in supervise_image_bytes:
                        supervise_img = torchvision.io.decode_jpeg(supervise_image_byte,
                                                                   mode=torchvision.io.ImageReadMode.RGB,
                                                                   device=torch.device(
                                                                       device)).float() / 255.0  # (3, H, W)
                        decoded_supervise_images.append(supervise_img)
                    sr_supervise_image = torch.stack(decoded_supervise_images, dim=0)  # (n_views, 3, H, W)
                    sr_supervise_image_list.append(sr_supervise_image)
                sr_supervise_images = torch.stack(sr_supervise_image_list, dim=0)  # (batch_size, n_views, 3, H, W)
                ...
                sr_supervise_masks = batch["supervise_masks"].to(device)
                sr_supervise_images_wo_bg = sr_supervise_images * sr_supervise_masks + (
                            1 - sr_supervise_masks) * bg_color  # (n_views, 3, H, W)
                supervise_images_wo_bg = interpolate_images(
                    sr_supervise_images_wo_bg, target_size=(config.img_size, config.img_size)
                )  # (n_views, 3, 518, 518)
                supervise_masks = interpolate_images(
                ...
                if sr_supervise_images_wo_bg.shape[-2] != config.sr_img_size or sr_supervise_images_wo_bg.shape[
                    -1] != config.sr_img_size:
                    sr_supervise_images_wo_bg = interpolate_images(
                        sr_supervise_images_wo_bg, target_size=(config.sr_img_size, config.sr_img_size)
                    )
            ...
            if rank == 0 and batch_idx % 20 == 0:
                ...
                save_rendered_images(
                    combined_output.detach().cpu().clone(),
                    f"{config.render_images_path}/val",
                    epoch,
                    batch_idx,
                )
