...
def pose_encoding_to_extri_intri(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        cx = image_size_hw[1] / 2.0  # Default to center of width
        cy = image_size_hw[0] / 2.0
        ...
        if build_intrinsics:
            ...
            intrinsics = torch.zeros(pose_encoding.shape[:-1] + (3, 3), device=pose_encoding.device) if pose_encoding.dim() > 1 else torch.zeros((3, 3), device=pose_encoding.device)
            intrinsics[..., 0, 0] = fx
            intrinsics[..., 1, 1] = fy
            intrinsics[..., 0, 2] = cx
            intrinsics[..., 1, 2] = cy
