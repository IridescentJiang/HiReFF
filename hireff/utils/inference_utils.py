"""Shared utilities for HiReFF inference scripts.
...
def load_model(checkpoint_path: str, device: str | None = None) -> tuple[HiReFF, str]:
    """Load a HiReFF model from a local checkpoint file.
    ...
    model = HiReFF.from_checkpoint(checkpoint_path)
    model.eval()
...
def read_dna_npz_entry(
    ...
    with np.load(npz_path, allow_pickle=True) as archive:
        for view in view_ids:
            ...
            # Invert camera-to-world to world-to-camera (HiReFF convention).
            extrinsics.append(
