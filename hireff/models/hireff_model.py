...
from hireff.models.aggregator import Aggregator
from hireff.heads.camera_head import CameraHead
from hireff.heads.dpt_head import DPTHead
from hireff.heads.hireff_dpt_gs_head import HiReFF_DPT_GS_Head
from hireff.heads.gs_adaptor import process_gs_map
from hireff.utils.depth2points import unproject_depth_map_to_point_map
from hireff.utils.pose_enc import pose_encoding_to_extri_intri
from hireff.rendering.render_image import adjust_transl
from hireff.utils.interpolate import interpolate_images


class HiReFF(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024):
        ...
        self.gs_para_head = HiReFF_DPT_GS_Head(
            dim_in=2048,
            patch_size=(14, 14),
    ...
    def forward(
        ...
        """
        Forward pass of the HiReFF model.
    ...
    def from_checkpoint(cls, pretrained_path, map_location="cpu"):
        """
        Load pretrained parameters from a local file.

        Args:
            pretrained_path: Local model file path
            map_location: Device to load onto [cpu/cuda]
        """
        model = cls()  # Initialize empty model

        # Load checkpoint
        checkpoint = torch.load(pretrained_path, map_location=map_location)

        # Auto-detect common checkpoint formats
        ...
        # Strip multi-GPU training prefix from parameter names
        state_dict = {
            k.replace("module.", ""): v
