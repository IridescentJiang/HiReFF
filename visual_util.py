...
def predictions_to_ply(
    ...
    Args:
        ...
        conf_thres (float): Percentage of low-confidence points to filter out
        filter_by_frames (str): Frame filter specification
        mask_black_bg (bool): Mask out black background pixels
        mask_white_bg (bool): Mask out white background pixels
        mask_sky (bool): Apply sky segmentation mask
        target_dir (str): Output directory for intermediate files
        prediction_mode (str): Prediction mode selector

    Returns:
        trimesh.Trimesh: Point cloud data in PLY format
